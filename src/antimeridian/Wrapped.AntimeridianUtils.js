...
export function calculateAntimeridianLat(latLngA, latLngB) {
  if (latLngA instanceof L.LatLng && latLngB instanceof L.LatLng) {
    ...
    throw new Error(
      "In order to calculate the Antimeridian latitude, two valid LatLngs are required."
    );
  }
}
...
export function isCrossMeridian(latLngA, latLngB) {
  if (latLngA instanceof L.LatLng && latLngB instanceof L.LatLng) {
    ...
    throw new Error(
      "In order to calculate whether two LatLngs cross a meridian, two valid LatLngs are required."
    );
  }
}
...
export function pushLatLng(ring, projectedBounds, latlng, map) {
  if (
    ring instanceof Array &&
    projectedBounds instanceof L.Bounds &&
    latlng instanceof L.LatLng &&
    map instanceof L.Map
  ) {
    ring.push(map.latLngToLayerPoint(latlng));
    projectedBounds.extend(ring[ring.length - 1]);
  } else {
    throw new Error(
      "In order to push a LatLng into a ring, the ring point array, the LatLng, the projectedBounds, and the map must all be valid."
    );
  }
}
...
export function isBreakRing(latLngA, latLngB) {
  if (latLngA instanceof L.LatLng && latLngB instanceof L.LatLng) {
    ...
    throw new Error(
      "In order to calculate whether the ring created by two LatLngs should be broken, two valid LatLngs are required."
    );
  }
}
...
export function breakRing(currentLat, nextLat, rings, projectedBounds, map) {
  if (
    currentLat instanceof L.LatLng &&
    nextLat instanceof L.LatLng &&
    rings instanceof Array &&
    projectedBounds instanceof L.Bounds &&
    map instanceof L.Map
  ) {
    ...
    throw new Error(
      "In order to break a ring, all the inputs must exist and be valid."
    );
  }
}
